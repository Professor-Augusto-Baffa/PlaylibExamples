...
void MainLoop(void)
{
	...
	update(dt);
	//LoadMapa("Mapa01.txt");
	KeyboardExecute(&scene, graphics); //processa o teclado. Tudo relativo a teclado é processado aqui
	//printf ("x = %d, y = %d\n", mouse_x, mouse_y);
...
//	for (int g= 0; g < arma.pente.resistencia_max; g++)  //carrega todos os possiveis tiros na tela. Repare que é um vetor de 7 espaços, 
...
void draw()
{
	...
	else if (scene == 1)
	{
		...
		//	     
  //      int x, y;
  //      for (x = 0; x < 30; x++)//NÃO ENTENDI PORQUE COLOCAR 30 AQUI
  //      {
  //          for (y = 0; y < 26; y++)
  //          {
  //              if (Mapa[x][y] == 'x')
...
void controla_arma(Heroi personagem, Equipamento *equipamento, Projetil *projetil, Sprite_sheet matriz_projetil[][10], Image sheet_projetil)
{
	...
	if (equipamento->pente.municao_disponivel)
	{
		...
		if (cont_seg == 4) //tempo de desaparecimento do projetil
		{
			projetil->disparo = false;
			cont_seg = 0;
		}

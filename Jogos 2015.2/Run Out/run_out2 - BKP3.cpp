...
//bool key_states[256]; //ver como funciona essa funcao, não entendi direito
...
void load ()
{
	...
	for (int g= 0; g < 6; g++)  //carrega todos os possiveis tiros na tela. Repare que é um vetor de 7 espaços, 
	{													//ou seja, o maximo possivel de frangos na tela é 7 
  //      projetil_magico[g].sheet_projetil.LoadPNGImage("Data\\Imagens\\tiro.png");// carrega a imagem da bala e a matriz do sprite no estruct da bala em si
		//InitSprites(&projetil_magico[g], 13, 10, 1, 2);
		projetil_magico[g].disparo = false;
		projetil_magico[g].velocidade = 200;
		projetil_magico[g].timer_projetil = false;
		projetil_magico[g].tempo_projetil = 0;
    }
...
void controla_arma(Heroi personagem, Equipamento *equipamento, Projetil *projetil, Sprite_sheet matriz_projetil[][10], Image sheet_projetil)
{
	...
	if (KeyState['h'] && equipamento->status == false)
	{
		...
		for(z=0;z<i ;z++)
		{
				...
				if (projetil[z].disparo==false)
				{
					projetil[z].disparo = true;
					projetil[z].timer_projetil = true;
					projetil[z].x = personagem.x;
					projetil[z].y = personagem.y;

					if (personagem.animation_clip==0)
						projetil[z].direcao = 0;
					else if (personagem.animation_clip==1)
						projetil[z].direcao = 1;
					else if (personagem.animation_clip==2)
						projetil[z].direcao = 2;
					else if (personagem.animation_clip==3)
						projetil[z].direcao = 3;
					break;
				}
		}
		KeyState['h'] = false;
	...
	for (z = 0; z < i; z ++)
		{
			if(projetil[z].disparo == true)
			{


				if (projetil[z].direcao == 0)
				{
					projetil[z].y -= dt * projetil[z].velocidade;
					pos = 1;
				}
		
				else if (projetil[z].direcao == 1)
				{
					projetil[z].x -= dt * projetil[z].velocidade;
					ajuste_y = 20;
					pos = 2;
				}
		
				else if (projetil[z].direcao == 2)
				{
					projetil[z].x += dt * projetil[z].velocidade; 
					ajuste_y = 20;
					pos = 0;
		...
		if (Collision(projetil[z].x+10, projetil[z].y+10, monstro.x, monstro.y, monstro.x+56, monstro.y+80)) //tempo de desaparecimento do projetil
		{
			projetil[z].disparo = false;

			if (Collision(projetil[z].x+10, projetil[z].y+10, monstro.x, monstro.y, monstro.x+56, monstro.y+80))
			{
				projetil[z].disparo = false;
		...
		if (projetil[z].disparo)
			graphics.DrawImage2D(projetil[z].x, projetil[z].y+ajuste_y, 32, 32, matriz_projetil[0][pos].x, matriz_projetil[0][pos].y, matriz_projetil[0][pos].width, matriz_projetil[0][pos].height, sheet_projetil);
...
void MoveEnemy1()
{
	  ...
	  if (next_frame)
    {
	  ...
	  {
		  if (Collision(projetil_magico[g].x+10, projetil_magico[g].y+10, monstro.x, monstro.y, monstro.x+56, monstro.y+80))
		  {
				monstro.vida--;
				projetil_magico[g].disparo=false;
		  }

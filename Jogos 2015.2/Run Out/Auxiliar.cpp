...
int Delay(float tempo_atraso, int tipo) //0 - atraso, 1 - pause
{

	static int controle = 1; //pause ativo
	static float tempo_corrido;
	tempo_corrido += graphics.GetElapsedTime();

	if (tipo == 0) //ATRASO
	{

		if  (tempo_corrido <= tempo_atraso)
		{
			 tempo_corrido += graphics.GetElapsedTime();
			 printf("Tempo corrido\n = %f", tempo_corrido);
			 return 1;
		}
